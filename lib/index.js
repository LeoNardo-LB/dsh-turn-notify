...
export const Config = Schema.object({
    ...
    windowsToastAumid: Schema.union([
        Schema.const('auto').description('注册自有 AUMID（开始菜单 dsh 快捷方式）并以 dsh 名义显示 toast；注册/校验失败自动回退 PowerShell AUMID'),
 ...
 */
export function buildToastScript(title, body, launchUrl, audio = 'system', aumidSetup = '') {
 ...
 * 调用方维持各自回退，不影响通知显示。
 */
export function buildEnsureAumidScript(aumid) {
    const csharp = [
        'using System;',
 ...
 */
export function buildBalloonScript(title, body, sessionId, clickUrl, deepLink, waitSeconds, soundOn, aumidSetup = '') {
